...
async def test_list_users_as_admin(client, create_user, auth_header_factory):
    admin = await create_user("admin@example.com", "StrongPass123", role=user_role("ADMIN"))
...
async def test_update_self_profile(client, create_user, auth_header_factory):
    ...
    headers = auth_header_factory(user)
    resp = await client.put(f"/users/{user.id}", json={"full_name": "Edited"}, headers=headers)
    assert resp.status_code == 200
    assert resp.json()["full_name"] == "Edited"


@pytest.mark.asyncio
async def test_update_profile_admin_or_self_only(client, create_user, auth_header_factory):
    ...
    resp = await client.put(f"/users/{user2.id}", json={"full_name": "Hack"}, headers=headers)
    assert resp.status_code == 403
    assert resp.json().get("error", {}).get("code") == "AUTH_003"
...
@pytest.mark.asyncio
async def test_admin_update_role_and_status_and_delete(client, create_user, auth_header_factory):
    admin = await create_user("admin2@example.com", "StrongPass123", role=user_role("ADMIN"))
    headers = auth_header_factory(admin)
    target = await create_user("t@example.com", "StrongPass123")

    # role
    resp = await client.put(f"/users/{target.id}/role", json={"role": "ADMIN"}, headers=headers)
